...
import {
  ...
  BeneCopyTableHistoryEntry,
  BeneCopyService,
} from '../../services/bene-copy.service';
import { EnvironmentService } from '../../services/environment.service';

@Component({
  selector: 'app-bene-copy',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './bene-copy.component.html',
  styleUrl: './bene-copy.component.scss',
})
export class BeneCopyComponent implements OnInit {
  ...
  readonly form = this.formBuilder.nonNullable.group({
    ...
    validators: [BeneCopyComponent.environmentsMustDifferValidator],
  });
  ...
  async copy(): Promise<void> {
    ...
    if (this.form.invalid) {
      this.errorMessage.set('Choose different source and target environments and enter a numeric Bene Link Key.');
      return;
    }
    ...
    try {
      const response = await firstValueFrom(
        this.beneCopyService.submitJob(this.form.getRawValue()),
      );
  ...
  private async loadHistory(preferredJobId?: string): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(this.beneCopyService.getHistory(this.historyPageSize, 'legacy'));
